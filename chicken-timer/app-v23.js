(function (global) {
  ...
  function buildSlot(def) {
    ...
    const counterStage = document.createElement("div");
    counterStage.className = "v23-counter-stage";
    counterStage.hidden = true;
    timerPanel.appendChild(counterStage);

    const readoutCluster = document.createElement("div");
    readoutCluster.className = "readout-cluster";
    readoutCluster.classList.add("v23-readout-target", "is-selected");
    readoutCluster.dataset.targetKind = "primary";
    readoutCluster.dataset.slotInteractive = "true";
    readoutCluster.tabIndex = 0;
    readoutCluster.setAttribute("role", "button");
    readoutCluster.setAttribute("aria-pressed", "true");
    bindPress(readoutCluster, () => selectV23Target(view, "primary"));
    counterStage.appendChild(readoutCluster);
    ...
    actions.className = "v23-six-actions";
    actions.dataset.targetKind = "primary";
    timerPanel.appendChild(actions);
    const adjustButtons = new Map();
    const actionDefinitions = [
      { action: "minus10", label: "-10", amount: "10", arrows: "▼", deltaSeconds: -10, tone: "down" },
      { action: "minus60", label: "-60", amount: "60", arrows: "▼▼", deltaSeconds: -60, tone: "down" },
      { action: "delete", label: "삭제", word: "삭제", tone: "down" },
      { action: "plus10", label: "+10", amount: "10", arrows: "▲", deltaSeconds: 10, tone: "up" },
      { action: "plus60", label: "+60", amount: "60", arrows: "▲▲", deltaSeconds: 60, tone: "up" },
      { action: "add", label: "추가", word: "추가", tone: "up" },
    ];
    actionDefinitions.forEach((control) => {
      const button = document.createElement("button");
      button.type = "button";
      button.className = `v23-action v23-${control.action} is-${control.tone}`;
      button.dataset.action = control.action;
      if (Number.isFinite(control.deltaSeconds)) button.dataset.deltaSeconds = String(control.deltaSeconds);
      setAccessibleName(button, control.label);
      if (control.arrows) {
        const arrows = document.createElement("span");
        arrows.className = "v23-action-arrows";
        arrows.setAttribute("aria-hidden", "true");
        arrows.textContent = control.arrows;
        const amount = document.createElement("span");
        amount.className = "v23-action-amount";
        amount.setAttribute("aria-hidden", "true");
        amount.textContent = control.amount;
        button.append(arrows, amount);
      } else {
        const word = document.createElement("span");
        word.className = "v23-action-word";
        word.textContent = control.word;
        button.appendChild(word);
      }
      bindPress(button, () => invokeV23Action(view, def.id, control));
      actions.appendChild(button);
      adjustButtons.set(control.action, button);
    });
    const splitAction = adjustButtons.get("add");
    ...
    view = {
      ...
      adjustControls: counterStage,
      counterStage,
      readoutCluster,
      adjustButtons,
      actions,
      splitAction,
      v23SelectedTarget: { kind: "primary", entryId: "" },
    };

    document.documentElement.dataset.countdownRuntimeV23 = "native";
    sealV23Geometry(view);

    return view;
  }

  function sealV23Geometry(view) {
    if (!view) return;
    const important = (element, property, value) => element && element.style
      && element.style.setProperty(property, value, "important");
    important(view.timerPanel, "display", "grid");
    important(view.timerPanel, "grid-template-rows", "minmax(0,1fr) clamp(1.2rem,12%,1.8rem)");
    important(view.timerPanel, "grid-template-columns", "minmax(0,1fr)");
    important(view.timerPanel, "grid-template-areas", "none");
    important(view.counterStage, "display", "grid");
    important(view.counterStage, "grid-column", "1");
    important(view.counterStage, "grid-row", "1");
    important(view.readoutCluster, "position", "relative");
    important(view.readoutCluster, "inset", "auto");
    important(view.readoutCluster, "width", "100%");
    important(view.readoutCluster, "height", "100%");
    important(view.readoutCluster, "inline-size", "100%");
    important(view.readoutCluster, "block-size", "100%");
    important(view.readoutCluster, "max-width", "none");
    important(view.readoutCluster, "max-height", "none");
    important(view.readoutCluster, "justify-self", "stretch");
    important(view.readoutCluster, "align-self", "stretch");
    important(view.readoutCluster, "padding", "0");
    important(view.readout, "position", "relative");
    important(view.readout, "inset", "auto");
    important(view.readout, "transform", "none");
    important(view.readout, "width", "100%");
    important(view.readout, "height", "100%");
    important(view.readout, "inline-size", "100%");
    important(view.readout, "block-size", "100%");
    important(view.readout, "max-inline-size", "none");
    important(view.readout, "max-block-size", "none");
    important(view.readout, "max-width", "none");
    important(view.readout, "max-height", "none");
    important(view.actions, "display", "grid");
    important(view.actions, "grid-column", "1 / -1");
    important(view.actions, "grid-row", "2");
  }
  ...
  function selectV23Target(view, kind, entryId) {
    if (!view) return false;
    const safeKind = kind === "aux" ? "aux" : "primary";
    const safeEntryId = safeKind === "aux" ? String(entryId || "") : "";
    if (safeKind === "aux" && !safeEntryId) return false;
    view.v23SelectedTarget = { kind: safeKind, entryId: safeEntryId };
    const targets = [view.readoutCluster].concat(
      view.splitList ? Array.from(view.splitList.querySelectorAll(".split-readout-cluster")) : [],
    );
    targets.forEach((target) => {
      const selected = target.dataset.targetKind === safeKind
        && (safeKind === "primary" || target.dataset.entryId === safeEntryId);
      target.classList.toggle("is-selected", selected);
      target.setAttribute("aria-pressed", selected ? "true" : "false");
    });
    refreshV23Actions(view, getMainSlot(view.card.dataset.slotId));
    requestReadoutFit();
    return true;
  }

  function refreshV23Actions(view, slot) {
    if (!view || !view.actions) return;
    const target = view.v23SelectedTarget || { kind: "primary", entryId: "" };
    const entries = splitStore && slot ? splitStore.getEntries(slot.id) : [];
    const aux = target.kind === "aux" ? entries.find((entry) => entry && entry.id === target.entryId) : null;
    if (target.kind === "aux" && !aux) {
      view.v23SelectedTarget = { kind: "primary", entryId: "" };
      return selectV23Target(view, "primary");
    }
    const running = target.kind === "aux"
      ? Boolean(aux && aux.status === "running")
      : Boolean(slot && slot.status === "running");
    view.actions.dataset.targetKind = target.kind;
    Array.from(view.actions.querySelectorAll(".v23-action")).forEach((button) => {
      const action = button.dataset.action;
      button.disabled = action === "add"
        ? target.kind === "aux" || !running || entries.length > 0
        : action === "delete" ? !(slot && slot.status !== "idle") : !running;
      if (action === "add") {
        button.setAttribute("aria-label", target.kind === "aux" ? "보조 타이머 추가 사용 불가" : "보조 타이머 추가");
      }
    });
  }

  function invokeV23Action(view, slotId, control) {
    if (!view || !control) return false;
    const target = view.v23SelectedTarget || { kind: "primary", entryId: "" };
    if (target.kind === "aux") {
      if (control.action === "add") return false;
      if (control.action === "delete") return handleClearSplitTimer(view, slotId, target.entryId);
      return handleAdjustSplitTimer(view, slotId, target.entryId, control.deltaSeconds);
    }
    if (control.action === "delete") return clearMainTimer(slotId);
    if (control.action === "add") {
      toggleAddMode(view);
      return true;
    }
    return handleAdjustTimer(view, slotId, control.deltaSeconds, control.label);
  }
  ...
  function fitTimerReadout(view, options) {
    if (document.documentElement.dataset.countdownRuntimeV23 === "native") {
      fitV23CardReadouts(view);
      return;
    }
  ...
  function fitV23CardReadouts(view) {
    if (!view || !view.card || view.timerPanel.hidden) return;
    const targets = [view.readoutCluster].concat(
      view.splitList ? Array.from(view.splitList.querySelectorAll(".split-readout-cluster")) : [],
    ).filter((target) => target && target.offsetParent);
    if (targets.length === 0) return;
    const measured = targets.map((target) => {
      const readout = target.querySelector(".timer-readout, .split-timer-readout");
      const backing = readout && readout.querySelector(".v23-glyph-backing");
      if (!readout || !backing) return 8;
      readout.dataset.counterLayout = "two-line";
      let low = 8;
      let high = Math.min(180, Math.max(8, target.clientHeight * .62));
      for (let index = 0; index < 10; index += 1) {
        const middle = (low + high) / 2;
        readout.style.setProperty("--v23-fitted-font-size", `${middle}px`);
        readout.style.setProperty("font-size", `${middle}px`, "important");
        const targetBox = target.getBoundingClientRect();
        const range = document.createRange();
        range.selectNodeContents(backing);
        const glyphBox = range.getBoundingClientRect();
        const fits = glyphBox.width <= targetBox.width - 4
          && glyphBox.height <= targetBox.height - 4
          && glyphBox.left >= targetBox.left + 1
          && glyphBox.right <= targetBox.right - 1
          && glyphBox.top >= targetBox.top + 1
          && glyphBox.bottom <= targetBox.bottom - 1;
        if (fits) low = middle; else high = middle;
      }
      return Math.max(8, Math.floor(low));
    });
    const sharedFont = Math.max(8, Math.min(...measured) - 2);
    targets.forEach((target) => {
      const readout = target.querySelector(".timer-readout, .split-timer-readout");
      if (readout) {
        readout.style.setProperty("--v23-fitted-font-size", `${sharedFont}px`);
        readout.style.setProperty("font-size", `${sharedFont}px`, "important");
        readout.style.setProperty("line-height", "1", "important");
      }
    });
  }
  ...
  function updateSlotView(view, slot, splitEntriesOverride) {
    ...
    view.counterStage.hidden = false;
  ...
  function setCountdownReadout(readout, text) {
    ...
    minutes.className = "v23-counter-line v23-minutes";
    minutes.textContent = match[1];
    const separator = document.createElement("span");
    separator.className = "v23-counter-separator";
    separator.textContent = ":";
    const seconds = document.createElement("span");
    seconds.className = "v23-counter-line v23-seconds";
    seconds.textContent = match[2];
    const backing = document.createElement("span");
    backing.className = "v23-glyph-backing";
    backing.append(minutes, separator, seconds);
    backing.style.setProperty("position", "relative", "important");
    backing.style.setProperty("inset", "auto", "important");
    backing.style.setProperty("transform", "none", "important");
    backing.style.setProperty("display", "grid", "important");
    backing.style.setProperty("grid-template-rows", "repeat(2,minmax(0,1fr))", "important");
    [minutes, seconds].forEach((line) => {
      line.style.setProperty("position", "static", "important");
      line.style.setProperty("inset", "auto", "important");
    ...
    readout.replaceChildren(backing);
  }
  ...
  function renderSplitView(view, slot, splitEntriesOverride) {
    ...
    view.timerPanel.style.setProperty("grid-template-columns", nextHasSplit ? "repeat(2,minmax(0,1fr))" : "minmax(0,1fr)", "important");
    view.splitSection.style.setProperty("display", nextHasSplit ? "block" : "none", "important");
    view.splitSection.style.setProperty("grid-column", "2", "important");
    view.splitSection.style.setProperty("grid-row", "1", "important");
    view.splitSection.style.setProperty("width", "100%", "important");
    view.splitSection.style.setProperty("height", "100%", "important");
    view.splitList.style.setProperty("display", nextHasSplit ? "grid" : "none", "important");
    view.splitList.style.setProperty("width", "100%", "important");
    view.splitList.style.setProperty("height", "100%", "important");
    setReadoutLayoutMode(view);
    ...
    if (canPatchCards) {
      ...
      const target = view.v23SelectedTarget || { kind: "primary", entryId: "" };
      selectV23Target(view, target.kind, target.entryId);
      return;
    }
  ...
  function createSplitTimerCard(view, slotId, entry) {
    ...
    card.style.setProperty("grid-template", "minmax(0,1fr) / minmax(0,1fr)", "important");
    ...
    const readoutCluster = document.createElement("div");
    readoutCluster.className = "split-readout-cluster v23-readout-target";
    readoutCluster.dataset.targetKind = "aux";
    readoutCluster.dataset.entryId = entry.id;
    readoutCluster.tabIndex = 0;
    readoutCluster.setAttribute("role", "button");
    readoutCluster.setAttribute("aria-pressed", "false");
    bindPress(readoutCluster, () => selectV23Target(view, "aux", entry.id));
    readoutCluster.style.setProperty("grid-area", "1 / 1", "important");
    readoutCluster.style.setProperty("max-width", "none", "important");
    readoutCluster.style.setProperty("max-height", "none", "important");
    readoutCluster.style.setProperty("justify-self", "stretch", "important");
    readoutCluster.style.setProperty("align-self", "stretch", "important");
    readoutCluster.style.setProperty("padding", "0", "important");
    readoutCluster.style.setProperty("inline-size", "100%", "important");
    readoutCluster.style.setProperty("block-size", "100%", "important");
    readoutCluster.appendChild(head);

    const readout = document.createElement("div");
    readout.className = "split-timer-readout";
    readoutCluster.appendChild(readout);
    [readoutCluster, readout].forEach((element) => {
      ...
      element.style.setProperty("block-size", "100%", "important");
  ...
  function updateSplitTimerCard(card, slotId, entry) {
    ...
    card.dataset.entryId = entry.id;
    if (fields.readoutCluster) fields.readoutCluster.dataset.entryId = entry.id;
